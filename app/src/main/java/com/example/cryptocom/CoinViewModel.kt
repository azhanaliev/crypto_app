...
import com.example.cryptocom.api.ApiFactory
import com.example.cryptocom.database.AppDatabase
import com.example.cryptocom.pojo.CoinPriceInfo
import com.example.cryptocom.pojo.CoinPriceInfoRawData
...
class CoinViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun loadData() {
        val disposable: Disposable = ApiFactory.apiService.getTopCoinsInfo(limit = 50)
            .map { it -> it.data?.map { it.coinInfo?.name }?.joinToString(",").toString() }
